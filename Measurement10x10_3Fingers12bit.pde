...
final int PIXEL_SIZE_X=WINDOW_SIZE_X/FINGER_NUM/(SENSOR_X_NUM+1);
...
void setup() {  
  ...
  myPort.buffer(FINGER_NUM*(SENSOR_X_NUM*SENSOR_Y_NUM+THERMAL_NUM)*3/2+2);//12bit 2 data are packed to 8bit 3 data. 
  ...
  for (finger=0; finger<FINGER_NUM; finger++) {
    ...
    for (x=0; x<SENSOR_X_NUM; x++) {
      for (y=0; y<SENSOR_Y_NUM; y++) {
        fill(20, PressureDistribution[finger][x][y]>>4, 20);
...
void serialEvent(Serial mp)
{
  ...
  for (finger=0; finger<FINGER_NUM; finger++) {
    ...
    for (x = 0; x<THERMAL_NUM; x++) {
      ...
      packing = (packing+1)%2; //0,1,0,1,0,1,...
      ThermalDistribution[finger][x] =0.0155 * (float)iThermalDist[finger][x] - 16.557; //Calculation by theoretical formula.
    }
  }
