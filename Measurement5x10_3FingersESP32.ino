/****************************************************************
  ...
  2020/3/14 Faster Data Transmission using better serial and multi-core programming
  ...
  while(1) {
    if (Serial.available() > 0) {
      ...
      if (rcv == PC_ESP32_MEASURE_REQUEST) {
        Serial.write(snd, (COL_NUM*ROW_NUM+THERMAL_NUM)*FINGER_NUM+2); //send 8bit time in milliseconds
